...
export const DICT = {
  en: {
    ...
    // play home
    homeHi: (n) => `Evening, ${n}`,
    homeSub: "From buy-in to the final count, Colour Up keeps the night straight — then hands you a receipt with the fewest transfers to settle up. It never touches anyone's money.",
    rehostTitle: "Re-host your last table",
    ...
    // lobby + invite
    // first-run explainer
    introTitle: "How Colour Up works", introReopen: "How this works",
    introStep1: "Host a table or join one", introStep1Sub: "Scan a QR or set your own stake.",
    introStep2: "Buy in, then play", introStep2Sub: "Everyone puts in the same stake to start.",
    introStep3: "Count your stack in private", introStep3Sub: "No one sees your number until every seat has counted.",
    introStep4: "Get a receipt", introStep4Sub: "The fewest transfers that settle everyone up.",
    introPromise: "Colour Up never touches money — no cards charged, no balances held. Just a receipt, and you settle up yourselves.",
    introCta: "Let's play", introSkip: "Skip",
    // step rail — short labels for the always-visible progress track
    railSetup: "Set up", railOpen: "Open", railBuyIn: "Buy in", railPlay: "Play",
    railCount: "Count", railCheck: "Check", railSettle: "Settle",
    stepOf: (i, n, label) => `Step ${i} of ${n} — ${label}`,
    stepDone: (label) => `${label} — done`, stepUpcoming: (label) => `${label} — upcoming`,
    ...
    reconnecting: "Reconnecting — catching up…",
    openTableFailed: (msg) => `Could not open the table: ${msg}`,
    alreadyStarted: "This table is already underway on the host's device.",
    // fund
    ...
    needAccountTitle: "Create your account",
    needAccountBlurb: "You're seated, but putting money in needs a real account — so your buy-ins and history follow you, not just this browser tab.",
    ...
    saveHistoryFailedLog: "Couldn't save this table to your history — the receipt above is still correct.",
    // done
    settled: "Settled", throughTable: (p) => `${p} through the table`,
    colourUpSummary: (seats, tr) => `${seats} seat${seats === 1 ? "" : "s"} settled in ${tr} transfer${tr === 1 ? "" : "s"}`,
    ...
    friendAddError: "Couldn't add that friend — check your connection and try again.",
    ...
    saveProfileError: "Couldn't save your profile — check your connection and try again.",
    ...
    // account
    account: "account", signedInAs: (e) => `Signed in as ${e}`, emailPlaceholder: "you@email.com",
    signOut: "Sign out", changePassword: "Change password",
    // auth (sign up / log in / reset)
    authTagline: "Your profile, friends, and history — with you on every device.",
    continueWithGoogle: "Continue with Google", orDivider: "or",
    signUpTitle: "Create your account", signInTitle: "Welcome back",
    emailLabel: "email", passwordLabel: "password", newPasswordLabel: "new password",
    signUpBtn: "Create account", signInBtn: "Log in",
    switchToSignIn: "Already have an account? Log in", switchToSignUp: "New here? Create an account",
    forgotPassword: "Forgot password?", backToLogin: "Back to log in",
    confirmEmailSent: "Check your inbox — confirm your email to finish creating your account.",
    resetTitle: "Reset your password", resetBlurb: "Enter the email you signed up with and we'll send a reset link.",
    sendResetLink: "Send reset link", resetSent: "Check your inbox for a password reset link.",
    setNewPasswordTitle: "Set a new password", setNewPasswordBtn: "Save new password", passwordUpdated: "Password updated.",
    // pay sheet
  ...
  ru: {
    ...
    homeHi: (n) => `Добрый вечер, ${n}`,
    homeSub: "От взноса до финального подсчёта Colour Up ведёт весь вечер, а в конце выдаёт чек с минимальным числом переводов, чтобы все сошлись. Деньги через приложение никогда не проходят.",
    rehostTitle: "Пересобрать прошлый стол",
    ...
    introTitle: "Как работает Colour Up", introReopen: "Как это работает",
    introStep1: "Соберите стол или зайдите к другим", introStep1Sub: "Отсканируйте QR или задайте свою ставку.",
    introStep2: "Внесите взнос и играйте", introStep2Sub: "Все начинают с одинаковой ставки.",
    introStep3: "Посчитайте стек приватно", introStep3Sub: "Никто не видит ваше число, пока все не подсчитают свои.",
    introStep4: "Получите чек", introStep4Sub: "Минимум переводов, чтобы все сошлись.",
    introPromise: "Colour Up никогда не работает с деньгами — карты не списываются, балансы не хранятся. Только чек, а расчёт — между вами.",
    introCta: "Играть", introSkip: "Пропустить",
    railSetup: "Настройка", railOpen: "Открытие", railBuyIn: "Взнос", railPlay: "Игра",
    railCount: "Подсчёт", railCheck: "Проверка", railSettle: "Расчёт",
    stepOf: (i, n, label) => `Шаг ${i} из ${n} — ${label}`,
    stepDone: (label) => `${label} — готово`, stepUpcoming: (label) => `${label} — впереди`,
    ...
    waitingForHost: "Ждём, пока банкир раздаст карты.",
    reconnecting: "Переподключение — синхронизируем…",
    openTableFailed: (msg) => `Не удалось открыть стол: ${msg}`,
    alreadyStarted: "Игра за этим столом уже идёт на устройстве банкира.",
    ...
    needAccountTitle: "Создайте аккаунт",
    needAccountBlurb: "Вы заняли место, но чтобы внести деньги, нужен настоящий аккаунт — тогда ваши входы и история сохранятся за вами, а не только в этой вкладке.",
    ...
    saveHistoryFailedLog: "Не удалось сохранить стол в историю — чек выше всё равно верен.",
    settled: "Рассчитано", throughTable: (p) => `${p} прошло через стол`,
    colourUpSummary: (seats, tr) => `${seats} мест${seats === 1 ? "о" : seats < 5 ? "а" : ""} сошлись в ${tr} перевод${tr === 1 ? "" : tr < 5 ? "а" : "ов"}`,
    ...
    friendAddError: "Не удалось добавить друга — проверьте соединение и попробуйте снова.",
    ...
    saveProfileError: "Не удалось сохранить профиль — проверьте соединение и попробуйте снова.",
    ...
    account: "аккаунт", signedInAs: (e) => `Вход выполнен как ${e}`, emailPlaceholder: "you@email.com",
    signOut: "Выйти", changePassword: "Изменить пароль",
    authTagline: "Ваш профиль, друзья и история — на любом устройстве.",
    continueWithGoogle: "Продолжить через Google", orDivider: "или",
    signUpTitle: "Создать аккаунт", signInTitle: "С возвращением",
    emailLabel: "email", passwordLabel: "пароль", newPasswordLabel: "новый пароль",
    signUpBtn: "Создать аккаунт", signInBtn: "Войти",
    switchToSignIn: "Уже есть аккаунт? Войти", switchToSignUp: "Впервые здесь? Создать аккаунт",
    forgotPassword: "Забыли пароль?", backToLogin: "Назад ко входу",
    confirmEmailSent: "Проверьте почту — подтвердите email, чтобы завершить создание аккаунта.",
    resetTitle: "Сброс пароля", resetBlurb: "Введите email, на который зарегистрирован аккаунт — пришлём ссылку для сброса.",
    sendResetLink: "Отправить ссылку", resetSent: "Проверьте почту — там ссылка для сброса пароля.",
    setNewPasswordTitle: "Новый пароль", setNewPasswordBtn: "Сохранить новый пароль", passwordUpdated: "Пароль обновлён.",
