...
export function useGameData(session) {
  ...
  // 'connected' | 'reconnecting' — surfaces a Realtime channel drop (wifi
  // loss, a backgrounded phone suspending its socket) so the UI can show
  // it instead of silently going stale. See the resync effect below for
  // what actually recovers from it.
  const [connectionStatus, setConnectionStatus] = useState("connected");

  const gameIdRef = useRef(gameId);
  useEffect(() => { gameIdRef.current = gameId; }, [gameId]);

  // Pulls the full current state for `id` fresh from the server. Used for
  // the initial load, and — critically — for resyncing after a dropped
  // connection: Realtime only pushes events for changes that happen while
  // actually connected, so anything that changed during a drop is
  // otherwise silently missed forever, not just delayed. Returns whether it
  // actually succeeded — callers use that (not just "we tried") to decide
  // whether the connection is really back, since Supabase-js resolves with
  // an `error` field rather than rejecting on a network failure.
  const refetchAll = useCallback(async (id) => {
    if (!id) return false;
    const [{ data: gameRow, error: gErr }, { data: seatRows, error: sErr }, { data: entryRows, error: eErr }, { data: countRows, error: cErr }] = await Promise.all([
      ...
      supabase.from("game_players").select("*").eq("game_id", id).order("joined_at", { ascending: true }),
      supabase.from("entries").select("*").eq("game_id", id).order("created_at", { ascending: true }),
      supabase.from("player_counts").select("*").eq("game_id", id),
    ]);
    if (gErr || sErr || eErr || cErr) { setError((gErr || sErr || eErr || cErr).message); return false; }
    setGame(gameRow);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let everSubscribed = false; // first SUBSCRIBED is the normal startup, not a "reconnect"

    (async () => {
      setLoading(true);
      await refetchAll(gameId);
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        ...
        if (status === "SUBSCRIBED") {
          if (everSubscribed) resync(); // a real reconnect — catch up on whatever was missed
    ...
    // "Reconnecting" only clears once a refetch actually *succeeds* — not
    // just once we've tried one. Supabase-js resolves with an `error` field
    // rather than throwing on a network failure, so a naive "we called
    // refetchAll" would flip the banner off even while still genuinely
    // offline. This is the single place that's allowed to declare
    // "connected" again, shared by the channel-status handler above and
    // the online/visibility listeners below, so the two can't disagree.
    async function resync() {
      const ok = await refetchAll(gameId);
      if (!cancelled && ok) setConnectionStatus("connected");
    }

    // Belt-and-braces beyond the channel's own status: a backgrounded phone
    // can suspend its socket for a while before the channel reports
    // anything, and `navigator.onLine` flipping is an even earlier signal
    // than Supabase's own reconnect logic.
    function onOffline() { if (!cancelled) setConnectionStatus("reconnecting"); }
  ...
  return {
    gameId, setGameId, game, seats, entries, playerCounts, viewer, isHost, allAgreed, allFunded, loading, error, connectionStatus,
